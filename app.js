(function(){
  "use strict";
  const engine=window.PBSEngine,programs=window.PBS_PROGRAMS,template=window.PBS_DAY_TEMPLATE,$=id=>document.getElementById(id);
  const els={clock:$("stationClock"),mode:$("modeLabel"),title:$("nowTitle"),programTime:$("programTime"),enter:$("enterButton"),stationCard:$("stationCard"),cardLabel:$("stationCardLabel"),cardTitle:$("stationCardTitle"),cardCountdown:$("stationCardCountdown"),stationWatch:$("stationWatch"),startOver:$("startOverButton"),rewind:$("rewindButton"),live:$("liveButton"),position:$("positionLabel"),remaining:$("remainingLabel"),progress:$("progressBar"),next:$("nextCards"),guide:$("guideRows"),guideDate:$("guideDate"),share:$("shareButton"),shareStatus:$("shareStatus")};
  ...
  function hideStationWatch(){els.stationWatch.hidden=true;els.stationWatch.removeAttribute("href");}
  function showStationCard(state,message){
    els.stationCard.hidden=false;els.cardLabel.textContent=(state.block.movie.era||"").includes("Current")?"PBS · NEW & NOTED":"PBS · PUBLIC MEDIA";els.cardTitle.textContent=state.segment.title;
    if(state.segment.watchUrl){els.stationWatch.href=state.segment.watchUrl;els.stationWatch.hidden=false;els.cardCountdown.textContent=message||"This program opens from its available streaming source.";}
    else{hideStationWatch();els.cardCountdown.textContent=message||(state.segment.cleared?`${formatDuration(state.segmentRemaining)} until the next program`:"Archive slot · inline source not currently available.");}
  }
  function clearStartupTimer(){if(startupTimer){clearTimeout(startupTimer);startupTimer=0;}}
  function nudgePlayback(){clearStartupTimer();if(!entered||!playerReady||!startupPending||!player)return;let state;try{state=player.getPlayerState();}catch(_){return;}if(state===YT.PlayerState.PLAYING){startupPending=false;startupAttempts=0;return;}if(state===YT.PlayerState.PAUSED){startupPending=false;return;}if(state===YT.PlayerState.UNSTARTED||state===YT.PlayerState.CUED||state===YT.PlayerState.BUFFERING){if(startupAttempts<5){startupAttempts++;try{player.playVideo();}catch(_){}startupTimer=setTimeout(nudgePlayback,startupAttempts===1?250:650);}}}
  function loadMedia(state){
    ...
    if(loadedKey!==mediaKey){loadedKey=mediaKey;loadedProgramVideoId=state.segment.videoId;startupPending=false;startupAttempts=0;clearStartupTimer();const current=document.getElementById("player");const iframe=document.createElement("iframe");iframe.id="player";iframe.className="player";iframe.title=state.segment.title;iframe.allow="autoplay; encrypted-media; picture-in-picture; fullscreen";iframe.allowFullscreen=true;iframe.referrerPolicy="strict-origin-when-cross-origin";iframe.src="https://www.youtube.com/embed/"+encodeURIComponent(state.segment.videoId)+"?autoplay=1&playsinline=1&controls=1&rel=0&loop=1&playlist="+encodeURIComponent(state.segment.videoId)+"&origin="+encodeURIComponent(location.origin);current.replaceWith(iframe);player=null;playerReady=false;return;}
  ...
  function tick(){
    ...
    window.PBS_LIVE_CONTEXT={program:state.block.movie,stationSeconds:state.blockElapsed,mediaSeconds:state.mediaSeconds,playing:entered&&!!(state.segment.videoId&&state.segment.cleared&&!failedVideoIds.has(state.segment.videoId)),mode:mode,sourceUrl:state.segment.watchUrl||state.block.movie.watchUrl||"",updatedAt:Date.now()};
    if(!entered){const inlineReady=!!(state.segment.videoId&&state.segment.cleared&&!failedVideoIds.has(state.segment.videoId));if(inlineReady){els.enter.hidden=false;els.stationCard.hidden=true;els.enter.innerHTML="<span>Enter PBS</span><small>Join the program at the moment airing now</small>";}else{els.enter.hidden=true;showStationCard(state,"Inline playback is not available for this scheduled program. Open its official source below.");}}
    els.clock.textContent=`${formatStationTime(Date.now())} local`;els.mode.textContent=mode==="live"?"LIVE PBS":"TIME SHIFTED";els.title.textContent=state.block.movie.title;setProgramArt(state.block.movie);els.programTime.textContent=`${formatStationTime(state.block.startsAtMs)}–${formatStationTime(state.block.endsAtMs)}`;
    els.position.textContent=mode==="live"?"Synced with the viewer’s local PBS channel clock":`${formatDuration(state.blockElapsed)} from start`;els.remaining.textContent=`${formatDuration(state.blockRemaining)} remaining in slot`;els.progress.style.width=`${Math.min(100,(state.blockElapsed/state.block.blockSeconds)*100)}%`;
  ...
  function localShareCredit(reference){
    const attemptId=`pbs-share-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`,parse=(key,fallback)=>{try{return JSON.parse(localStorage.getItem(key))||fallback;}catch(_){return fallback;}},session=parse("starquest_session",null),users=parse("starquest_users",{}),signedIn=session&&session.key&&users[session.key],profile=signedIn||parse("starquest_guest_profile_v1",{key:"__guest__",username:"Guest",tokens:0,shareCount:0,pendingShareCredits:0,shareEvents:[],ledger:[]});
    profile.tokens=Math.max(0,Number(profile.tokens)||0);profile.shareCount=Math.max(0,Number(profile.shareCount)||0)+1;profile.pendingShareCredits=Math.max(0,Number(profile.pendingShareCredits)||0)+1;let awarded=0;while(profile.pendingShareCredits>=10){profile.pendingShareCredits-=10;profile.tokens+=1;awarded+=1;}
    const event={id:attemptId,contentId:reference,createdAt:Date.now(),confirmed:true};profile.shareEvents=(profile.shareEvents||[]).concat(event).slice(-250);profile.ledger=(profile.ledger||[]).concat({id:`tx-${attemptId}`,type:awarded?"share_reward":"share_credit",amount:awarded,balance:profile.tokens,pendingShareCredits:profile.pendingShareCredits,createdAt:Date.now()}).slice(-500);
    if(signedIn){users[session.key]=profile;localStorage.setItem("starquest_users",JSON.stringify(users));}else localStorage.setItem("starquest_guest_profile_v1",JSON.stringify(profile));return{awarded,progressToNextCoin:profile.pendingShareCredits,balance:profile.tokens};
  }
  async function shareChannel(){
    const title=els.title.textContent&&!els.title.textContent.includes("Loading")?els.title.textContent:document.title,share={title:`${title} · PBS`,text:`Watch ${title} on PBS — 1987 to now.`,url:location.href};
