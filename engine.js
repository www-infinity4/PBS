(function(root){
  ...
  function zonedToUtc(year,month,day,hour=0,minute=0,second=0){
    ...
    for(let i=0;i<4;i++){const p=stationParts(new Date(guess));const represented=Date.UTC(p.year,p.month-1,p.day,p.hour,p.minute,p.second);guess+=target-represented;}
    return guess;
  }
  ...
  function pickChoice(choices,key,index){if(!choices.length)return null;return choices[hash(`${key}:${index}`)%choices.length];}
  function createDaySchedule(nowMs,programs,template){
    const p=stationParts(new Date(nowMs));const midnightMs=zonedToUtc(p.year,p.month,p.day);const key=dateKey(nowMs);
    return template.map((slot,index)=>{const programKey=pickChoice(slot.choices,key,index);const program=programs[programKey];const blockSeconds=slot.duration*60;return{id:`${key}-${String(index).padStart(3,"0")}`,movie:program,startsAtMs:midnightMs+slot.minute*60000,endsAtMs:midnightMs+(slot.minute+slot.duration)*60000,blockSeconds};});
  }
  ...
  function resolve(nowMs,schedule){
    let block=schedule.find(item=>nowMs>=item.startsAtMs&&nowMs<item.endsAtMs);if(!block)block=nowMs<schedule[0].startsAtMs?schedule[0]:schedule[schedule.length-1];
    const blockElapsed=Math.max(0,Math.min(block.blockSeconds-1,Math.floor((nowMs-block.startsAtMs)/1000)));const segments=createSegments(block);const segment=segments.find(item=>blockElapsed>=item.stationStart&&blockElapsed<item.stationStart+item.duration)||segments[segments.length-1];const segmentElapsed=Math.max(0,blockElapsed-segment.stationStart);
